...
static int resolve_conflict(struct action *apx, struct action *apy)
{
    ...
    if (apx->type == SHIFT && apy->type == REDUCE) {
        ...
        assert(apx->type == SH_RESOLVED ||
               ...
               apy->type == RD_RESOLVED ||
               apy->type == SSCONFLICT || apy->type == SRCONFLICT || apy->type == RRCONFLICT);
...
static void parseonetoken(struct pstate *psp)
{
    ...
    case WAITING_FOR_DECL_OR_RULE:
        if (x[0] == '%') {
            ...
            if (psp->prevrule == 0) {
                ErrorMsg(psp->filename, psp->tokenlineno, "There is no prior rule upon which to attach the code \
fragment which begins on this line.");
                psp->errorcnt++;
            } else if (psp->prevrule->code != 0) {
                ErrorMsg(psp->filename, psp->tokenlineno, "Code fragment beginning on this line is not the first \
to follow the previous rule.");
    ...
    case PRECEDENCE_MARK_1:
        if (!isupper(x[0])) {
            ...
            ErrorMsg(psp->filename, psp->tokenlineno, "Precedence mark on this line is not the first \
to follow the previous rule.");
            psp->errorcnt++;
        } else {
    ...
    case LHS_ALIAS_3:
        if (x[0] == ':' && x[1] == ':' && x[2] == '=') {
            ...
            ErrorMsg(psp->filename, psp->tokenlineno,
                     "Missing \"->\" following: \"%s(%s)\".", psp->lhs->name, psp->lhsalias);
            psp->errorcnt++;
            psp->state = RESYNC_AFTER_RULE_ERROR;
        }
        break;
...
PRIVATE void translate_code(struct lemon *lemp, struct rule *rp)
{
    ...
    if (rp->lhsalias && !lhsused) {
        ErrorMsg(lemp->filename, rp->ruleline,
                 "Label \"%s\" for \"%s(%s)\" is never used.", rp->lhsalias, rp->lhs->name, rp->lhsalias);
        lemp->errorcnt++;
    }
    ...
    for (i = 0; i < rp->nrhs; i++) {
        if (rp->rhsalias[i] && !used[i]) {
            ErrorMsg(lemp->filename, rp->ruleline,
                     "Label %s for \"%s(%s)\" is never used.", rp->rhsalias[i], rp->rhs[i]->name, rp->rhsalias[i]);
            lemp->errorcnt++;
    ...
    for (i = 0; i < lemp->nsymbol; i++) {
        ...
        for (j = i + 1; j < lemp->nsymbol; j++) {
            ...
            if (sp2 && sp2->type != TERMINAL && sp2->destructor
                && sp2->dtnum == sp->dtnum && strcmp(sp->destructor, sp2->destructor) == 0) {
                fprintf(out, "    case %d: /* %s */\n", sp2->index, sp2->name);
                lineno++;
                sp2->destructor = 0;
            }
